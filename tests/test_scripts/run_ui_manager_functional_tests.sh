#!/bin/bash
# Script to run the UI Manager Functional Tests
...
echo -e "${YELLOW}Running UI Manager Functional Tests...${NC}"
...
if [ "$BUILD_TYPE" = "Debug" ]; then
  TEST_EXECUTABLE="$PROJECT_ROOT/bin/debug/ui_manager_functional_tests"
else
  TEST_EXECUTABLE="$PROJECT_ROOT/bin/release/ui_manager_functional_tests"
fi
...
OUTPUT_FILE="$PROJECT_ROOT/test_results/ui_manager_functional_tests_output.txt"
...
if [ $TEST_RESULT -ne 0 ] || grep -q "failure\|test cases failed\|fatal error" "$OUTPUT_FILE"; then
  ...
  echo -e "${GREEN}✅ All UI Manager Functional tests passed!${NC}"
  exit 0
fi
