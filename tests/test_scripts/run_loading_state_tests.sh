#!/bin/bash
# Script to run the Loading State Tests
...
echo -e "${YELLOW}Running Loading State Tests...${NC}"
...
if [ "$BUILD_TYPE" = "Debug" ]; then
  TEST_EXECUTABLE="$PROJECT_ROOT/bin/debug/loading_state_tests"
else
  TEST_EXECUTABLE="$PROJECT_ROOT/bin/release/loading_state_tests"
fi
...
OUTPUT_FILE="$PROJECT_ROOT/test_results/loading_state_tests_output.txt"
...
if [ $TEST_RESULT -ne 0 ] || grep -q "failure\|test cases failed\|fatal error" "$OUTPUT_FILE"; then
  ...
  echo -e "${GREEN}✅ All Loading State tests passed!${NC}"
  exit 0
fi
