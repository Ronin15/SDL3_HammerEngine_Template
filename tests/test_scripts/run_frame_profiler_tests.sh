...
# Helper script to build and run FrameProfiler tests
...
for arg in "$@"; do
  case $arg in
    --verbose)
      ...
      ;;
    --help)
      echo -e "${BLUE}FrameProfiler Test Runner${NC}"
      echo -e "Usage: ./run_frame_profiler_tests.sh [options]"
      echo -e "\nOptions:"
...
echo -e "${BLUE}Running FrameProfiler tests...${NC}"

# Define the test executables to run
EXECUTABLES=("frame_profiler_tests")
...
if [ "$FINAL_RESULT" -ne 0 ]; then
  echo -e "\n${RED}Some FrameProfiler tests failed!${NC}"
  exit $FINAL_RESULT
else
  echo -e "\n${GREEN}All FrameProfiler tests completed successfully!${NC}"
  echo -e "${GREEN}✓ frame_profiler_tests${NC}"
  exit 0
fi
