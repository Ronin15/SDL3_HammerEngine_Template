#!/bin/bash
# Script to run the Buffer Reuse Tests
...
echo -e "${YELLOW}Running Buffer Reuse Tests...${NC}"
...
if [ "$BUILD_TYPE" = "Debug" ]; then
  TEST_EXECUTABLE="$PROJECT_ROOT/bin/debug/buffer_reuse_tests"
else
  TEST_EXECUTABLE="$PROJECT_ROOT/bin/release/buffer_reuse_tests"
fi
...
OUTPUT_FILE="$PROJECT_ROOT/test_results/buffer_reuse_tests_output.txt"
...
if [ $TEST_RESULT -ne 0 ] || grep -q "failure\|test cases failed\|fatal error" "$OUTPUT_FILE"; then
  ...
  echo -e "${GREEN}✅ All Buffer Reuse tests passed!${NC}"
  exit 0
fi
