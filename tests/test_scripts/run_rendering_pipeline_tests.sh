...
echo -e "${YELLOW}Running Rendering Pipeline Tests...${NC}"
...
if [ "$BUILD_TYPE" = "Debug" ]; then
  TEST_EXECUTABLE="$PROJECT_ROOT/bin/debug/rendering_pipeline_tests"
else
  TEST_EXECUTABLE="$PROJECT_ROOT/bin/release/rendering_pipeline_tests"
fi
...
OUTPUT_FILE="$PROJECT_ROOT/test_results/rendering_pipeline_tests_output.txt"
...
if [ $TEST_RESULT -ne 0 ] || grep -q "failure\|test cases failed\|fatal error" "$OUTPUT_FILE"; then
  ...
  echo -e "${GREEN}✅ All Rendering Pipeline tests passed!${NC}"
  exit 0
fi
