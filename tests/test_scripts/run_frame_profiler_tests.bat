...
if /i "%~1"=="--help" (
    echo !BLUE!FrameProfiler Test Runner!NC!
    echo Usage: run_frame_profiler_tests.bat [options]
    echo.
    echo Options:
...
echo !BLUE!Running FrameProfiler Tests!NC!

rem Track overall result
set OVERALL_RESULT=0

call :run_single_test frame_profiler_tests
...
if !OVERALL_RESULT! neq 0 (
    echo.
    echo !RED!Some FrameProfiler tests failed!!NC!
    exit /b 1
) else (
    echo.
    echo !GREEN!All FrameProfiler tests completed successfully!!NC!
    echo !GREEN!  frame_profiler_tests!NC!
    exit /b 0
)
