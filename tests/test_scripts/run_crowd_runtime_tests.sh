...
# Helper script to build and run Crowd Runtime tests (AI crowd behavior)
...
for arg in "$@"; do
  case $arg in
    --verbose)
      ...
      ;;
    --help)
      echo -e "${BLUE}Crowd Runtime Test Runner${NC}"
      echo -e "Usage: ./run_crowd_runtime_tests.sh [options]"
      echo -e "\nOptions:"
...
echo -e "${BLUE}Running Crowd Runtime tests...${NC}"

# Define the test executables to run
EXECUTABLES=("crowd_runtime_tests")
...
if [ "$FINAL_RESULT" -ne 0 ]; then
  echo -e "\n${RED}Some Crowd Runtime tests failed!${NC}"
  exit $FINAL_RESULT
else
  echo -e "\n${GREEN}All Crowd Runtime tests completed successfully!${NC}"
  echo -e "${GREEN}✓ crowd_runtime_tests${NC}"
  exit 0
fi
