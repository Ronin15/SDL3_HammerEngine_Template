...
if "%SHOW_HELP%"=="true" (
    echo !CYAN!SettingsManager Test Runner!NC!
...
set "TEST_EXECUTABLE=..\..\bin\debug\settings_manager_tests.exe"
if not exist "!TEST_EXECUTABLE!" (
    set "TEST_EXECUTABLE=..\..\bin\debug\settings_manager_tests"
    if not exist "!TEST_EXECUTABLE!" (
        ...
        for /r ..\.. %%f in (settings_manager_tests settings_manager_tests.exe) do (
            if exist "%%f" (
        ...
        if "!FOUND_EXECUTABLE!"=="" (
            ...
            echo !YELLOW!Please build the project first with: ninja settings_manager_tests!NC!
            exit /b 1
        )
    )
)
...
set "LOG_FILE=settings_test_output_!TIMESTAMP!.log"
...
set "ROOT_TEST_EXECUTABLE=bin\debug\settings_manager_tests.exe"
...
copy "..\..\!LOG_FILE!" "!RESULTS_DIR!\settings_manager_test_output_!TIMESTAMP!.txt"
if !ERRORLEVEL! neq 0 (
    echo !RED!WARNING: Failed to copy test output to timestamped file!NC!
)

copy "..\..\!LOG_FILE!" "!RESULTS_DIR!\settings_manager_test_output.txt"
if !ERRORLEVEL! neq 0 (
...
echo === Test Cases Executed === > "!RESULTS_DIR!\settings_manager_test_cases.txt"
findstr /r /c:"Entering test case" "..\..\!LOG_FILE!" >> "!RESULTS_DIR!\settings_manager_test_cases.txt"
if !ERRORLEVEL! neq 0 (
...
findstr /r /c:"Entering test case" "..\..\!LOG_FILE!" > "!RESULTS_DIR!\settings_manager_test_cases_run.txt"
if !ERRORLEVEL! neq 0 (
...
echo   Primary: !RESULTS_DIR!\settings_manager_test_output.txt
echo   Timestamped: !RESULTS_DIR!\settings_manager_test_output_!TIMESTAMP!.txt

:: Show test cases that were run
echo.
echo !BLUE!Test Cases Executed:!NC!
if exist "!RESULTS_DIR!\settings_manager_test_cases_run.txt" (
    for /f "usebackq delims=" %%l in ("!RESULTS_DIR!\settings_manager_test_cases_run.txt") do (
        set "line=%%l"
...
if "!TEST_PASSED!"=="false" (
    ...
    echo   !RESULTS_DIR!\settings_manager_test_output.txt
)
