@echo off
:: EntityStateManager Test Runner
...
if /i "%~1"=="--basic-test" (
    set "TEST_FILTER=--run_test=*Add*,*Has*,*GetCurrent*"
    shift
    goto :parse_args
)
if /i "%~1"=="--transition-test" (
    set "TEST_FILTER=--run_test=*SetState*,*Transition*"
    shift
    goto :parse_args
)
if /i "%~1"=="--update-test" (
    set "TEST_FILTER=--run_test=*Update*"
    shift
    goto :parse_args
)
shift
...
if "%SHOW_HELP%"=="true" (
    echo !CYAN!EntityStateManager Test Runner!NC!
    ...
    echo   --basic-test         Run only basic state management tests
    echo   --transition-test    Run only state transition tests
    echo   --update-test        Run only update propagation tests
...
set "TEST_EXECUTABLE=..\..\bin\debug\entity_state_manager_tests.exe"
if not exist "!TEST_EXECUTABLE!" (
    set "TEST_EXECUTABLE=..\..\bin\debug\entity_state_manager_tests"
    if not exist "!TEST_EXECUTABLE!" (
        ...
        for /r ..\.. %%f in (entity_state_manager_tests entity_state_manager_tests.exe) do (
            if exist "%%f" (
        ...
        if "!FOUND_EXECUTABLE!"=="" (
            ...
            echo !YELLOW!Please build the project first with: ninja entity_state_manager_tests!NC!
            exit /b 1
        )
    )
)
...
set "LOG_FILE=entity_state_manager_test_output_!TIMESTAMP!.log"
...
set "ROOT_TEST_EXECUTABLE=bin\debug\entity_state_manager_tests.exe"
...
copy "..\..\!LOG_FILE!" "!RESULTS_DIR!\entity_state_manager_test_output_!TIMESTAMP!.txt"
if !ERRORLEVEL! neq 0 (
    echo !RED!WARNING: Failed to copy test output to timestamped file!NC!
)

copy "..\..\!LOG_FILE!" "!RESULTS_DIR!\entity_state_manager_test_output.txt"
if !ERRORLEVEL! neq 0 (
...
echo === Test Cases Executed === > "!RESULTS_DIR!\entity_state_manager_test_cases.txt"
findstr /r /c:"Entering test case" "..\..\!LOG_FILE!" >> "!RESULTS_DIR!\entity_state_manager_test_cases.txt"
if !ERRORLEVEL! neq 0 (
...
findstr /r /c:"Entering test case" "..\..\!LOG_FILE!" > "!RESULTS_DIR!\entity_state_manager_test_cases_run.txt"
if !ERRORLEVEL! neq 0 (
...
echo   Primary: !RESULTS_DIR!\entity_state_manager_test_output.txt
echo   Timestamped: !RESULTS_DIR!\entity_state_manager_test_output_!TIMESTAMP!.txt

:: Show test cases that were run
echo.
echo !BLUE!Test Cases Executed:!NC!
if exist "!RESULTS_DIR!\entity_state_manager_test_cases_run.txt" (
    for /f "usebackq delims=" %%l in ("!RESULTS_DIR!\entity_state_manager_test_cases_run.txt") do (
        set "line=%%l"
...
if "!TEST_PASSED!"=="false" (
    ...
    echo   !RESULTS_DIR!\entity_state_manager_test_output.txt
)
