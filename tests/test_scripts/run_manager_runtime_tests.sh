...
# Helper script to build and run Manager Runtime tests (FontManager, SoundManager, TextureManager)
...
for arg in "$@"; do
  case $arg in
    --verbose)
      ...
      ;;
    --help)
      echo -e "${BLUE}Manager Runtime Test Runner${NC}"
      echo -e "Usage: ./run_manager_runtime_tests.sh [options]"
      echo -e "\nOptions:"
      echo -e "  --verbose      Run tests with verbose output"
      echo -e "  --help         Show this help message"
      echo -e "\nTest Suites:"
      echo -e "  SoundManager runtime tests (init, volume, load/clear SFX and music)"
      echo -e "  FontManager runtime tests (load, reload, measure, wrap, clear)"
      echo -e "  TextureManager runtime tests (GPU load without device, clean after fail)"
      exit 0
      ;;
  esac
done

echo -e "${BLUE}Running Manager Runtime tests...${NC}"

# Define the test executables to run
EXECUTABLES=("manager_runtime_tests")
...
if [ "$FINAL_RESULT" -ne 0 ]; then
  echo -e "\n${RED}Some Manager Runtime tests failed!${NC}"
  exit $FINAL_RESULT
else
  echo -e "\n${GREEN}All Manager Runtime tests completed successfully!${NC}"
  echo -e "${GREEN}✓ SoundManager runtime tests${NC}"
  echo -e "${GREEN}✓ FontManager runtime tests${NC}"
  echo -e "${GREEN}✓ TextureManager runtime tests${NC}"
  exit 0
fi
