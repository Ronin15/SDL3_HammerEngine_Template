...
# Test configuration
declare -A ALL_TESTS=(
    ["memory_critical"]="buffer_utilization_tests buffer_reuse_tests event_manager_tests event_manager_behavior_tests event_coordination_integration_tests ai_optimization_tests ai_collision_integration_tests ai_manager_edm_integration_tests particle_manager_core_tests particle_manager_performance_tests particle_manager_threading_tests particle_manager_weather_tests behavior_functionality_tests entity_data_manager_tests entity_state_manager_tests resource_architecture_tests world_resource_manager_tests resource_template_manager_tests resource_integration_tests resource_factory_tests resource_edge_case_tests resource_change_event_tests resource_template_manager_json_tests inventory_component_tests json_reader_tests save_manager_tests settings_manager_tests game_state_manager_tests loading_state_tests world_generator_tests world_manager_tests world_manager_event_integration_tests collision_system_tests collision_manager_edm_integration_tests pathfinding_system_tests pathfinder_manager_tests pathfinder_manager_edm_integration_tests pathfinder_ai_contention_tests collision_pathfinding_integration_tests camera_tests rendering_pipeline_tests input_manager_tests ui_manager_functional_tests"
    ["thread_safety"]="thread_safe_ai_manager_tests thread_safe_ai_integration_tests ai_collision_integration_tests ai_manager_edm_integration_tests particle_manager_threading_tests thread_system_tests event_manager_tests event_manager_behavior_tests event_coordination_integration_tests resource_integration_tests resource_architecture_tests world_resource_manager_tests resource_template_manager_tests resource_factory_tests resource_edge_case_tests save_manager_tests game_state_manager_tests world_manager_tests world_manager_event_integration_tests collision_system_tests collision_manager_edm_integration_tests pathfinding_system_tests pathfinder_manager_tests pathfinder_manager_edm_integration_tests pathfinder_ai_contention_tests collision_pathfinding_integration_tests background_simulation_manager_tests"
    ["performance"]="event_manager_tests event_manager_behavior_tests event_coordination_integration_tests ai_optimization_tests ai_collision_integration_tests save_manager_tests particle_manager_performance_tests behavior_functionality_tests json_reader_tests resource_template_manager_tests resource_architecture_tests inventory_component_tests world_resource_manager_tests resource_change_event_tests resource_template_manager_json_tests resource_edge_case_tests game_state_manager_tests world_generator_tests world_manager_event_integration_tests world_manager_tests collision_system_tests pathfinding_system_tests pathfinder_manager_tests pathfinder_ai_contention_tests collision_pathfinding_integration_tests simd_correctness_tests rendering_pipeline_tests"
    ["comprehensive"]="event_types_tests weather_event_tests weather_controller_tests particle_manager_weather_tests thread_system_tests resource_change_event_tests inventory_component_tests resource_factory_tests resource_architecture_tests resource_template_manager_json_tests save_manager_tests settings_manager_tests game_state_manager_tests loading_state_tests world_generator_tests world_manager_event_integration_tests world_manager_tests collision_system_tests pathfinding_system_tests pathfinder_manager_tests collision_pathfinding_integration_tests controller_registry_tests day_night_controller_tests game_time_manager_tests game_time_manager_calendar_tests game_time_manager_season_tests camera_tests input_manager_tests ui_manager_functional_tests background_simulation_manager_tests"
)
...
run_cache_analysis() {
    ...
    for test in $cache_tests; do
        ...
        if [[ -f "${log_file}" ]]; then
            ...
            #   - EXCELLENT: L1I < 1%, L1D < 3%  |  GOOD: L1I < 2%, L1D < 5%
            local assessment="UNKNOWN"
            if [[ "${l1i_miss}" != "N/A" && "${l1d_miss}" != "N/A" ]]; then
                if (( $(echo "${l1i_miss} < 1.0 && ${l1d_miss} < 3.0" | bc -l 2>/dev/null || echo 0) )); then
                    assessment="EXCEPTIONAL"
                    ((exceptional_count++))
                elif (( $(echo "${l1i_miss} < 2.0 && ${l1d_miss} < 5.0" | bc -l 2>/dev/null || echo 0) )); then
                    assessment="GOOD"
                    ((good_count++))
                else
                    assessment="AVERAGE"
