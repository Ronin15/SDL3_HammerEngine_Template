...
struct ParticleManagerPerformanceFixture {
    ...
    void createParticles(size_t targetCount, const std::string& effectType = "Rain") {
        ...
        while (manager->getActiveParticleCount() < targetCount) {
            ...
            Vector2D position(
                basePosition.getX() + (effectIds.size() % 10 - 5) * 100,
        ...
        std::cout << "Created " << manager->getActiveParticleCount() 
                  << " particles using " << effectIds.size() << " effects" << std::endl;
    }
};

// Test update performance with 1000 particles
BOOST_FIXTURE_TEST_CASE(TestUpdatePerformance1000Particles, ParticleManagerPerformanceFixture) {
    ...
    std::cout << "Testing update performance with " << actualParticles << " particles" << std::endl;
...
BOOST_FIXTURE_TEST_CASE(TestUpdatePerformance5000Particles, ParticleManagerPerformanceFixture) {
...
BOOST_FIXTURE_TEST_CASE(TestParticleCreationThroughput, ParticleManagerPerformanceFixture) {
    ...
    std::cout << "Time to create " << NUM_EFFECTS << " effects: " << creationTime << "ms" << std::endl;
    ...
    std::cout << "Time for 10 updates (particle emission): " << emissionTime << "ms" << std::endl;
    std::cout << "Particles created: " << manager->getActiveParticleCount() << std::endl;
    
    BOOST_CHECK_GT(manager->getActiveParticleCount(), 0);
}

// Test memory usage scaling
BOOST_FIXTURE_TEST_CASE(TestMemoryUsageScaling, ParticleManagerPerformanceFixture) {
    Vector2D testPosition(500, 300);
    ...
    for (int batch = 0; batch < 5; ++batch) {
        // Add more effects
        for (int i = 0; i < 10; ++i) {
            Vector2D pos(testPosition.getX() + batch * 100 + i * 10, testPosition.getY());
            manager->playEffect("Rain", pos, 1.0f);
        }
        ...
        std::cout << "Particles: " << currentCount << ", Update time: " << updateTime << "ms" << std::endl;
    }
    ...
    double particleRatio = static_cast<double>(particleCounts.back()) / particleCounts.front();
    double timeRatio = lastTime / firstTime;
    
    std::cout << "Particle ratio: " << particleRatio << ", Time ratio: " << timeRatio << std::endl;
    
    // Time scaling should be reasonable (not more than 3x worse than linear)
    BOOST_CHECK_LT(timeRatio, particleRatio * 3.0);
}

// Test cleanup performance
BOOST_FIXTURE_TEST_CASE(TestCleanupPerformance, ParticleManagerPerformanceFixture) {
    ...
    BOOST_CHECK_LT(finalCount, initialCount); // Should have fewer particles after cleanup
}

// Test effect management performance
BOOST_FIXTURE_TEST_CASE(TestEffectManagementPerformance, ParticleManagerPerformanceFixture) {
    Vector2D testPosition(500, 300);
    ...
    std::cout << "Time to create and manage " << NUM_EFFECTS << " effects: " 
              << effectManagementTime << "ms" << std::endl;
...
BOOST_FIXTURE_TEST_CASE(TestSustainedPerformance, ParticleManagerPerformanceFixture) {
    ...
    for (int frame = 0; frame < NUM_FRAMES; ++frame) {
        double frameTime = measureExecutionTime([this]() {
            manager->update(0.016f);
        });
    ...
    std::cout << "Sustained performance over " << NUM_FRAMES << " frames:" << std::endl;
...
BOOST_FIXTURE_TEST_CASE(TestDifferentEffectTypesPerformance, ParticleManagerPerformanceFixture) {
    Vector2D testPosition(500, 300);
    
    std::vector<std::string> effectTypes = {"Rain", "Snow", "Fog"};
    
    for (const auto& effectType : effectTypes) {
        std::cout << "\nTesting " << effectType << " effect performance:" << std::endl;
