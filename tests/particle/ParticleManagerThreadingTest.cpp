...
struct ParticleManagerThreadingFixture {
    ParticleManagerThreadingFixture() {
        ...
        threadSystem = &Hammer::ThreadSystem::Instance();
        ...
        if (threadSystem->isShutdown() || threadSystem->getThreadCount() == 0) {
            ...
            if (!initSuccess) {
                // If initialization failed, this might be because it's already initialized
                // Check if it's working by verifying thread count
        ...
        if (threadSystem->isShutdown()) {
            throw std::runtime_error("ThreadSystem is shutdown and cannot be used for threading tests");
        }
    ...
    Hammer::ThreadSystem* threadSystem;
};

// Test concurrent particle creation
BOOST_FIXTURE_TEST_CASE(TestConcurrentParticleCreation, ParticleManagerThreadingFixture) {
    ...
    for (int threadId = 0; threadId < NUM_THREADS; ++threadId) {
        auto future = threadSystem->enqueueTaskWithResult([=, this, &successCount]() -> void {
        ...
        }, Hammer::TaskPriority::Normal, "ParticleCreationTask");
    ...
    std::cout << "Created " << manager->getActiveParticleCount() << " particles from " 
              << successCount.load() << " effects across " << NUM_THREADS << " threads" << std::endl;
}

// Test concurrent particle updates
BOOST_FIXTURE_TEST_CASE(TestConcurrentParticleUpdates, ParticleManagerThreadingFixture) {
    ...
    for (int threadId = 0; threadId < NUM_UPDATE_THREADS; ++threadId) {
        auto future = threadSystem->enqueueTaskWithResult([=, this, &updateCount]() -> void {
        ...
        }, Hammer::TaskPriority::Normal, "ParticleUpdateTask");
    ...
    BOOST_CHECK_EQUAL(updateCount.load(), NUM_UPDATE_THREADS * UPDATES_PER_THREAD);
...
BOOST_FIXTURE_TEST_CASE(TestThreadSafeEffectManagement, ParticleManagerThreadingFixture) {
    const int NUM_THREADS = 4;
    ...
    for (int threadId = 0; threadId < NUM_THREADS; ++threadId) {
        auto future = threadSystem->enqueueTaskWithResult([=, this, &effectsCreated, &effectsStopped, &createdEffectIds, &effectIdsMutex]() -> void {
            ...
            // Store remaining effect IDs for cleanup
            {
                ...
                createdEffectIds.insert(createdEffectIds.end(), localEffectIds.begin(), localEffectIds.end());
            }
        }, Hammer::TaskPriority::Normal, "EffectManagementTask");
    ...
    std::cout << "Active particles: " << manager->getActiveParticleCount() << std::endl;
...
BOOST_FIXTURE_TEST_CASE(TestConcurrentWeatherChanges, ParticleManagerThreadingFixture) {
    ...
    for (int threadId = 0; threadId < NUM_THREADS; ++threadId) {
        auto future = threadSystem->enqueueTaskWithResult([=, this, &weatherChanges, &weatherTypes]() -> void {
        ...
        }, Hammer::TaskPriority::Normal, "WeatherChangeTask");
    ...
    std::cout << "Weather changes completed: " << weatherChanges.load() << std::endl;
    std::cout << "Final particle count: " << manager->getActiveParticleCount() << std::endl;
    
    // Verify all weather changes were processed
    BOOST_CHECK_EQUAL(weatherChanges.load(), NUM_THREADS * WEATHER_CHANGES_PER_THREAD);
}

// Test concurrent access to performance stats
BOOST_FIXTURE_TEST_CASE(TestConcurrentStatsAccess, ParticleManagerThreadingFixture) {
    ...
    for (int threadId = 0; threadId < NUM_THREADS; ++threadId) {
        auto future = threadSystem->enqueueTaskWithResult([=, this, &statsReads, &updateCalls]() -> void {
        ...
        }, Hammer::TaskPriority::Normal, "StatsAccessTask");
...
BOOST_FIXTURE_TEST_CASE(TestThreadSafeCleanup, ParticleManagerThreadingFixture) {
    ...
    for (int threadId = 0; threadId < NUM_THREADS; ++threadId) {
        auto future = threadSystem->enqueueTaskWithResult([=, this, &cleanupStarted]() -> void {
            int operations = 0;
            while (!cleanupStarted.load(std::memory_order_acquire) && operations < 50) {
        ...
        }, Hammer::TaskPriority::Normal, "ContinuousOperationTask");
    ...
    // Weather effects should be significantly reduced after cleanup
    BOOST_CHECK_LT(finalCount, initialCount * 0.8); // Should have at least 20% fewer particles
}

// Test mixed concurrent operations
BOOST_FIXTURE_TEST_CASE(TestMixedConcurrentOperations, ParticleManagerThreadingFixture) {
    const int NUM_THREADS = 4;
    ...
    for (int threadId = 0; threadId < NUM_THREADS; ++threadId) {
        auto future = threadSystem->enqueueTaskWithResult([=, this, &totalOperations]() -> void {
            for (int i = 0; i < OPERATIONS_PER_THREAD; ++i) {
        ...
        }, Hammer::TaskPriority::Normal, "MixedOperationTask");
    ...
    std::cout << "Total mixed operations completed: " << totalOperations.load() << std::endl;
    std::cout << "Final active particles: " << manager->getActiveParticleCount() << std::endl;
    
    // Verify all operations completed
    BOOST_CHECK_EQUAL(totalOperations.load(), NUM_THREADS * OPERATIONS_PER_THREAD);
