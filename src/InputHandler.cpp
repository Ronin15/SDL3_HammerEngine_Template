...
void InputHandler::initializeGamePad() {
    ...
    if (gamepadIDs == nullptr) {
        std::cerr << "Failed to get gamepad IDs: " << SDL_GetError() << std::endl;
        return;
    }

    if (numGamepads > 0) {
        std::cout << "Number of Game Pads detected: " << numGamepads << std::endl;
        ...
        for (int i = 0; i < numGamepads; i++) {
            if (SDL_IsGamepad(gamepadIDs[i])) {
                ...
                if (gamepad) {
                    ...
                    std::cout << "Gamepad opened: " << SDL_GetGamepadName(gamepad) << std::endl;
                    ...
                    std::cerr << "Could not open gamepad: " << SDL_GetError() << std::endl;
                }
            }
        }
    } else {
...
bool InputHandler::getButtonState(int joy, int buttonNumber) const {
    if (joy < 0 || joy >= static_cast<int>(m_buttonStates.size()) ||
        buttonNumber < 0 || buttonNumber >= static_cast<int>(m_buttonStates[joy].size())) {
        return false;
    }
...
bool InputHandler::getMouseButtonState(int buttonNumber) const {
    if (buttonNumber < 0 || buttonNumber >= static_cast<int>(m_mouseButtonStates.size())) {
        return false;
    }
...
void InputHandler::onGamepadButtonDown(SDL_Event& event) {
    ...
    if (whichOne >= static_cast<int>(m_buttonStates.size()) ||
        event.gbutton.button >= static_cast<int>(m_buttonStates[whichOne].size())) {
        return;
    }
