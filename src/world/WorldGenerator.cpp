...
std::unique_ptr<WorldData> WorldGenerator::generateWorld(const WorldGenerationConfig& config) {
    GAMEENGINE_INFO("Generating world: " + std::to_string(config.width) + "x" + std::to_string(config.height) + " with seed " + std::to_string(config.seed));
...
void WorldGenerator::distributeObstacles(
    WorldData& world,
    const WorldGenerationConfig& config
) {
    int height = world.grid.size();
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            switch (tile.biome) {
                ...
                case Biome::SWAMP:
                    obstacleChance = 0.2f;
                    obstacleType = dist(rng) < 0.7f ? ObstacleType::TREE : ObstacleType::WATER;
                    break;
                case Biome::DESERT:
                    obstacleChance = 0.1f;
                    obstacleType = ObstacleType::ROCK;
                ...
                case Biome::HAUNTED:
                    obstacleChance = 0.3f;
                    obstacleType = dist(rng) < 0.6f ? ObstacleType::TREE : ObstacleType::ROCK;
                    break;
...
void WorldGenerator::calculateInitialResources(const WorldData& world) {
    ...
    GAMEENGINE_INFO("Initial resources - Trees: " + std::to_string(treeCount) + ", Rocks: " + std::to_string(rockCount) + ", Water: " + std::to_string(waterCount));
}
