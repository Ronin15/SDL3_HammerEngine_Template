...
public:
    ...
    explicit GuardBehavior(const Vector2D& guardPosition, float guardRadius = 200.0f, float alertRadius = 300.0f);
    
    // Constructor with mode
    explicit GuardBehavior(GuardMode mode, const Vector2D& guardPosition, float guardRadius = 200.0f);
...
private:
    ...
    struct EntityState {
        ...
        EntityState()
            : assignedPosition(0, 0)
            , lastKnownThreatPosition(0, 0)
            , investigationTarget(0, 0)
            , currentPatrolTarget(0, 0)
            , currentAlertLevel(AlertLevel::CALM)
            , currentMode(GuardMode::STATIC_GUARD)
            , lastThreatSighting(0)
            , alertStartTime(0)
    ...
    mutable std::uniform_real_distribution<float> m_radiusDistribution{0.3f, 1.0f};
    ...
    bool isThreatInFieldOfView(EntityPtr entity, EntityPtr threat, const EntityState& state) const;
    ...
    void updateAlertLevel(EntityPtr entity, EntityState& state, bool threatPresent);
    void handleThreatDetection(EntityPtr entity, EntityState& state, EntityPtr threat);
    ...
    bool isAtPosition(const Vector2D& currentPos, const Vector2D& targetPos, float threshold = 25.0f) const;
    ...
    void broadcastAlert(EntityPtr entity, AlertLevel level, const Vector2D& alertPosition);
};
