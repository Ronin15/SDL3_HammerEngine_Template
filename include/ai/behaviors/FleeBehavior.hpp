...
public:
    ...
    explicit FleeBehavior(float fleeSpeed = 4.0f, float detectionRange = 400.0f, float safeDistance = 600.0f);
    
    // Constructor with mode
    explicit FleeBehavior(FleeMode mode, float fleeSpeed = 4.0f, float detectionRange = 400.0f);
    ...
    void setStaminaSystem(bool enabled, float maxStamina = 100.0f, float staminaDrain = 10.0f);
...
private:
    ...
    struct EntityState {
        ...
        EntityState()
            : lastThreatPosition(0, 0)
            , fleeDirection(0, 0)
            , lastKnownSafeDirection(0, 0)
            , fleeStartTime(0)
    ...
    float m_boundaryPadding{100.0f}; // Distance from screen edge to consider unsafe
    ...
    mutable std::uniform_real_distribution<float> m_angleVariation{-0.5f, 0.5f}; // Radians
    ...
    Vector2D calculateFleeDirection(EntityPtr entity, EntityPtr threat, const EntityState& state);
    ...
    Vector2D avoidBoundaries(const Vector2D& position, const Vector2D& direction) const;
