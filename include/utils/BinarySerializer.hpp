...
namespace BinarySerial {
    ...
    public:
        ...
        bool writeVector(const std::vector<T>& vec) {
            ...
            if (size > 0) {
                m_stream->write(reinterpret_cast<const char*>(vec.data()), sizeof(T) * size);
            }
            return m_stream->good();
        }
    ...
    public:
        ...
        bool readString(std::string& str) {
            ...
            if (length > 1024 * 1024) { // 1MB limit
                SAVEGAME_ERROR("String length too large: " + std::to_string(length) + " bytes");
                return false;
            }
            
            str.resize(length);
            m_stream->read(&str[0], length);
            return m_stream->good() && m_stream->gcount() == static_cast<std::streamsize>(length);
        }
        ...
        bool readVector(std::vector<T>& vec) {
            ...
            if (size > 1024 * 1024) { // 1M elements limit
                SAVEGAME_ERROR("Vector size too large: " + std::to_string(size) + " elements");
                return false;
            }
            
            vec.resize(size);
            m_stream->read(reinterpret_cast<char*>(vec.data()), sizeof(T) * size);
            return m_stream->good() && m_stream->gcount() == static_cast<std::streamsize>(sizeof(T) * size);
        }
...
#define SERIALIZE_PRIMITIVE(writer, member) \
    if (!writer.write(member)) return false;

#define DESERIALIZE_PRIMITIVE(reader, member) \
    if (!reader.read(member)) return false;

#define SERIALIZE_STRING(writer, member) \
    if (!writer.writeString(member)) return false;
...
#define SERIALIZE_SERIALIZABLE(writer, member) \
    if (!writer.writeSerializable(member)) return false;

#define DESERIALIZE_SERIALIZABLE(reader, member) \
    if (!reader.readSerializable(member)) return false;
